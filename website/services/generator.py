    ...
    try:
        ...
        for syn_id in synsets:
            ...
            for lit in syn.literals:
                ...
                if (
                    clean_lit_lower != word_lower
                    and word_lower not in clean_lit_lower
                    and clean_lit_lower not in word_lower
                    and len(clean_lit.split()) <= 2
                    and "[" not in clean_lit
                ):
        ...
        if context_doc and target_token and nlp and nlp.vocab.has_vector:
            ...
            context_tokens = [
                ...
                if i != target_token.i
                and not context_doc[i].is_stop
                and context_doc[i].vector_norm > 0
            ]
            ...
            if context_tokens:
                ...
                for cand in candidate_literals:
                    ...
                    if cand_doc and cand_doc.vector_norm > 0:
                        ...
                        cos_sim_context = np.dot(cand_vec, context_vec) / (
                            cand_norm * context_norm + 1e-9
                        )

                        # Similarity to the original token
                        cos_sim_orig = np.dot(cand_vec, target_vec) / (
                            cand_norm * target_norm + 1e-9
                        )
                        ...
                        print(
                            f"[DEBUG]     Candidate '{cand}': context_sim={cos_sim_context:.3f}, meaning_sim={cos_sim_orig:.3f}, total={score:.3f}"
                        )
                ...
                print(
                    f"[DEBUG]   Could not calculate context vector for '{word}' (no surrounding tokens have vectors)"
                )
        ...
        if not best_syn and candidate_literals:
            ...
            print(
                f"[DEBUG] Falling back to random synonym for '{word}': '{best_syn}' (vectors unavailable or scoring failed)"
            )
        ...
        if best_syn and (best_score >= THRESHOLD or best_score == 0.0):
            print(
                f"[DEBUG] Best candidate for '{word}': '{best_syn}' (score: {best_score:.3f})"
            )
            return best_syn
        elif best_syn:
            print(
                f"[DEBUG] Rejected synonym '{best_syn}' for word '{word}' due to low score {best_score:.3f}"
            )
...
def apply_agreement(synonym_lemma, original_token):
    ...
    if original_token.pos_ == "VERB" and original_token.morph.get("VerbForm") == [
        "Part"
    ]:
    ...
    for morph_suff, text_suff in suffixes:
        if orig_text.endswith(text_suff):
            ...
            if not syn_gender:
                if synonym_lemma.endswith(("ă", "e", "ea")):
                    syn_gender = ["Fem"]
                else:
                    syn_gender = ["Masc"]
            ...
            if synonym_lemma.endswith("e"):
                ...
                if text_suff == "a":
                    return (
                        synonym_lemma + "ul"
                        if syn_gender == ["Masc"] or syn_gender == ["Neut"]
                        else synonym_lemma + "a"
                    )
                return synonym_lemma + text_suff
...
def generate_creative_sentence(keywords, examples, **kwargs) -> str:
    ...
    GENERAL_TEMPLATES = ["Na eroare bagamias pula"]
    ...
    template_text = random.choice(
        long_enough_examples if long_enough_examples else available_examples
    )
    ...
    print(
        f"[DEBUG] Identified {total_slots_count} potential slots in template: {template_text}"
    )
    ...
    for slot_idx in all_slots:
        ...
        syn = get_synonym(
            word_to_lookup, token.pos_, context_doc=doc, target_token=token
        )
        ...
        if syn:
            ...
            if inflected_syn != syn:
                print(
                    f"[DEBUG]   Inflected '{syn}' -> '{inflected_syn}' to match original '{token.text}'"
                )
                syn = inflected_syn
