...
def make_headers():
    hs = [
        ...
        {"name": "Host", "value": "www.scribens.com"},
        {"name": "Origin", "value": "https://www.scribens.com"},
        {"name": "Priority", "value": "u=4"},
        {"name": "Referer", "value": "https://www.scribens.com/ro/"},
    ...
    return {i["name"]: i["value"] for i in hs}
...
for text_path in glob.glob("data/02_segments/*.txt"):
    ...
    if json_out not in prev_errors:
        continue
    with open(json_out, "wb") as f:
        ...
        resp = requests.post(
            url=request_url, data=request_params, headers=request_headers
        )
