...
client = OpenAI(
    base_url="http://192.168.1.147:11434",
    api_key="pula"
)
...
SITE_CONFIG = {
    ...
    "azlyrics.com": {"selector": "div.main-page div.row div.col-xs-12 div:not([class])"},
...
NOISE_ELEMENTS = ["script", "style", "nav", "footer", "header", "aside", "form", "iframe", "button"]
...
def is_vanilla(text):
    ...
    dictionary_markers = [
        r'Etimologie:', r'Sinonime:', r'Definiție:', r'DEX - Dicționarul explicativ',
        r's\.f\.', r's\.m\.', r'adj\.', r'pl\.', r'Conjugare', r'Declonare', 
        r'plural:', r'singular:', r'articulat:'
    ]
    clinical_markers = [
        r'Simptome și tratament', r'Diagnostic:', r'Cauze:', r'Prevenție:',
        r'Sfatul medicului', r'Clinica', r'Tratarea cauzelor', r'Ghidul pacientului'
    ]
    ...
    try:
        ...
        if not content:
            common_selectors = [
                ...
                "#post-message", ".post_message", "#main-content", "article"
            ]
        ...
        if content:
            if len(content) < min_length:
                print(f"  Skipping URL (Content too short: {len(content)} chars): {url}")
                return None
...
def extract_injuraturi(url, content=None, keyword=None):
    ...
    try:
        ...
        sentences = [s.strip() for s in response.choices[0].message.content.split('\n') if s.strip()]
        ...
        for s in sentences:
            ...
            check_response = client.chat.completions.create(
              model=MODEL_NAME,
              messages=[
                {"role": "system", "content": "Raspunde doar cu 'DA' sau 'NU'. Acest text este o injuratura in limba romana? Ignora orice altceva."},
                {"role": "user", "content": s}
              ],
              max_tokens=10,
              temperature=0.0,
...
def save_to_db(url, sentences):
    ...
    try:
        ...
        if saved_count > 0:
            print(f"Successfully processed and saved {saved_count} sentences for: {url}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if not input_urls:
        print("Please provide at least one URL in injuraturi_urls.txt or via command line.")
    else:
